...
class fc(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        output = np.matmul(feat,self.params[self.w_name])
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        #############################################################################
        if(is_training): 
            '''p = 1.0-self.keep_prob
            kept = self.rng.binomial(1.0,p=p,size=feat.shape)
            if p != 0.0:
                kept = kept / p
            output = feat * kept'''
            p = self.keep_prob if self.keep_prob != 0 else 1.0
            kept = self.rng.binomial(1.0,p=p,size=feat.shape)
            kept = kept / p
            output = feat * kept
        else:
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        N,C = logit.shape
        loss = -1*np.sum(np.eye(C)[label]*np.log(logit))
...
def softmax(feat):
    ...
    #############################################################################
    f = np.exp(feat-np.max(feat))
    scores = f/np.sum(f,axis=-1)[:,np.newaxis]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return scores
