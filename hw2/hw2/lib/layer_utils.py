...
class VanillaRNN(object):
    ...
    def step_forward(self, x, prev_h):
        ...
        ############################################################################
        next_h = np.tanh(np.matmul(prev_h,self.params[self.wh_name]) + np.matmul(x,self.params[self.wx_name])+self.params[self.b_name])
    ...
    def step_backward(self, dnext_h, meta):
        ...
        dx = np.matmul(dnext_h*(1-next_h**2),self.params[self.wx_name].T)
        dprev_h = np.matmul(dnext_h*(1-next_h**2),self.params[self.wh_name].T)
        dWx = np.matmul(x.T,dnext_h*(1-next_h**2))
        dWh =np.matmul(prev_h.T,dnext_h*(1-next_h**2))
    ...
    def forward(self, x, h0):
        ...
        h = []
        for i in range(T):
            h0, m = self.step_forward(x[:,i,:],h0)
            h.append(h0)
            self.meta.append(m)
        h = np.array(np.transpose(h,axes=(1,0,2)))
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
        return h
    ...
    def backward(self, dh):
        ...
        N,T,D = dh.shape
        dx = []
        _, _, dWx_t, dWh_t, db_t = self.step_backward(dh[:,T-1,:],self.meta[T-1])
        self.grads[self.wx_name] = np.zeros_like(dWx_t)
        self.grads[self.wh_name] = np.zeros_like(dWh_t)
        self.grads[self.b_name] = np.zeros_like(db_t)
        dh0 = np.zeros((N,D))
        for i in reversed(range(T)):
            dh[:,i,:] += dh0
            dx_t, dh0, dWx_t, dWh_t, db_t = self.step_backward(dh[:,i,:],self.meta[i])
            dx.insert(0,dx_t)
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        a = np.matmul(x,self.params[self.wx_name]) + np.matmul(prev_h,self.params[self.wh_name]) + self.params[self.b_name]
        splice = np.int(a.shape[-1]/4)
        a_i,a_f,a_o,a_g = a[:,:splice],a[:,splice:2*splice],a[:,2*splice:3*splice],a[:,3*splice:4*splice]
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        di,df,do,dg = dc*g, dc*prev_c, dnext_h*np.tanh(next_c), dc*i 
    ...
    def forward(self, x, h0):
       ...
       # h = np.zeros((N, T, H))
        ...
        h = []
        for i in range(T):
            h0, c, meta = self.step_forward(x[:,i,:], h0, c)
            h.append(h0)
    ...
    def backward(self, dh):
        ...
        #############################################################################
        T = len(self.meta)
        dc = np.zeros(dh.shape[-1])
        self.grads[self.wx_name] = np.zeros_like(self.params[self.wx_name])
        self.grads[self.wh_name] = np.zeros_like(self.params[self.wh_name])
        self.grads[self.b_name] = np.zeros_like(self.params[self.b_name])
        dh0 = np.zeros((dh.shape[0],dh.shape[-1]))
        dx = np.zeros((dh.shape[0],dh.shape[1],self.grads[self.wx_name].shape[0]))
        for i in reversed(range(T)):
            dh_t = dh[:,i,:] + dh0 
            dx[:, i, :], dh0, dc, dWx, dWh, db = self.step_backward(dh_t, dc, self.meta[i])
...
class word_embedding(object):
    ...
    def forward(self, x):
        ...
        T = x.shape[1]
        self.meta = []
        out = np.zeros((x.shape[0],T,self.vec_dim))
        for i in range(T):
            x_v = np.identity(self.voc_dim)[x[:,i]]
            out[:,i,:] = np.matmul(x_v,self.params[self.w_name])
            self.meta.append(x_v)
        self.meta = np.transpose(self.meta,axes=(1,0,2))
    ...
    def backward(self, dout):
        ...
        # HINT: Look up the function np.add.at                                       #
        ##############################################################################
        self.grads[self.w_name] = np.zeros((self.meta.shape[-1],dout.shape[-1]))
        for i in range(dout.shape[1]):
            self.grads[self.w_name] += np.matmul(self.meta[:,i,:].T,dout[:,i,:])
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
